...
from flydance.analysis.arena import create_arena_from_yaml_data, plot_trajectory
import pickle
...
DEFAULT_ARENA_FILE = '../analysis/configs/big_arena_painted.pickle'
ARENA = None
if os.path.isfile(DEFAULT_ARENA_FILE):
    with open(DEFAULT_ARENA_FILE, 'rb') as f:
        ARENA = pickle.load(f)
    # print(ARENA.objects)
...
if __name__ == '__main__':
    ...
    layout_fname = 'layouts/fig_reloc_layout_0420l.svg'
    # fig1_fname = 'output/figure_reloc_0420_g.svg'
    fig1_fname = 'output/figure_reloc_0420l_winter.svg'

    #flyids = [6, 9, 22]
    flyids = [23, 22]  # rewarded (10 is also good), non-rewarded

    alltraj_fname = "/mnt/strawscience/anna/experiments/relocation/all_traj/all_ds_t01_cm.csv.gz"
    ...
    figure1(alltraj_fname, flyids, layout_fname, fig1_fname)

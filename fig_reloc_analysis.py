...
from flydance.analysis.arena import arena_hist2d, my_hist2d, plot_trajectory
import pickle
...
from plotting.plotting_helpers import my_gray_colormap, my_arrow, make_legend_arrow, conditions_mapping
...
def plot_traj_starts(layout, fname, traj_start_sec, **kwargs):
    ...
    ax.legend([vav, ar_reloc, ar_fr], ["flies", 'relocation', 'fictive RZ'],
...
def plot_fractions_dabest(layout, stats_fname, fr_color='blue', ar_color='red'):
    stats = pd.read_csv(stats_fname, sep='\t')
    stats.rename(index=str, columns={"at_reward": "actual RZ", "at_fictive_reward": "fictive RZ"}, inplace=True)
    swarm_palette = [fr_color, ar_color]
    ylims = {'rewarded': 0.35,
             'non-rewarded': 0.2}
    ...
    for condition, cond_label in conditions_mapping.items():
        ax = layout.axes[("group_fraction", "ax_frac_{}".format(cond_label))]
        ax_effect = layout.axes[("group_fraction", "ax_effect_{}".format(cond_label))]
        stats_loc = stats[(stats.condition == condition) & (stats.segment == 'test100')]

        fracs_paired = dabest.load(stats_loc, idx=("actual RZ", "fictive RZ"),
                                   paired=True, id_col='fly')

        fracs_paired.mean_diff.plot(axs=[ax, ax_effect], es_marker_size=5, swarm_ylim=(0, ylims[condition]))
...
if __name__ == '__main__':
    ...
    if MODE == 'swarm':
        layout_fname = 'layouts/fig2_layout_simpler.svg'
        stats_fname = '/mnt/strawscience/anna/experiments/relocation/stats/relocation_stats_time_fractions_no2.csv'  # melt: location: at_reward/at_fictive_reward
        fracplotfunc = plot_fractions
        fig_fname = 'output/reloc_analysis_sw.svg'
    elif MODE == 'scatter':
        # layout_fname = 'layouts/layout_reloc_analysis_sc.svg'
        # layout_fname = 'layouts/layout_reloc_analysis_sc_rot.svg'
        # layout_fname = 'layouts/layout_reloc_analysis_0507.svg'
        layout_fname = 'layouts/layout_reloc_analysis_0614.svg'

        stats_fname = '/mnt/strawscience/anna/experiments/relocation/stats/relocation_stats_t01_no2.csv'
        fig_fname = 'output/reloc_analysis_220711.svg'
    ...
    testper_fname = "/mnt/strawscience/anna/experiments/relocation/all_traj/test_cm_dt01.csv.gz"
    # mean_rz_fname = "/mnt/strawscience/anna/relocation/stats/mean_rewards_coords.csv"
    # mean_rz_fname = "mean_fictive_reward_coords_nofly2.csv"
    mean_rz_fname = "/mnt/strawscience/anna/experiments/relocation/reward_zones/mean_rewards_coords.csv"
